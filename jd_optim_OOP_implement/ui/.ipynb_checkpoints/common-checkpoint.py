...
def render_role_selector(state_manager):
    ...
    with st.container(border=True):
        ...
        with col1:
            ...
            # Simple one-row layout with default selection matching current role
            ...
            if selected_role != current_role:
                ...
                # Force a rerun to immediately apply the role change
                st.rerun()
        
        with col2:
            # Small status indicator for auto-save without cluttering the UI
            if state_manager.get('last_autosave_time'):
                last_save = datetime.datetime.fromtimestamp(state_manager.get('last_autosave_time'))
                time_diff = datetime.datetime.now() - last_save
                
                if time_diff.total_seconds() < 300:  # Within last 5 minutes
                    st.markdown("""
        ...
        with col3:
            # Toggle sidebar button - more prominent for better visibility
            st.markdown("<div style='padding-top: 8px;'></div>", unsafe_allow_html=True)
            if st.button("📋 History", use_container_width=True, key="toggle_history_btn"):
                # Set URL parameter to trigger sidebar toggle on refresh 
                st.experimental_set_query_params(sidebar_toggle="true")
                st.rerun()
...
def switch_tab(tab_name, state_manager):
    ...
    # Flag for auto-save when switching tabs
...
def render_role_specific_tabs(state_manager, tabs):
    """
...
def auto_save_session(state_manager, session_prefix="Auto"):
    """
    ...
    try:
        ...
        timestamp = datetime.datetime.now().isoformat()
        filename = f"autosave_{timestamp.replace(':', '_').replace('.', '_')}.json"
        filepath = os.path.join(history_dir, filename)
        
        # Create a clean session name with timestamp
        timestamp_formatted = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        session_name = f"{session_prefix} ({timestamp_formatted})"
        ...
        # Update last autosave time in state manager
        state_manager.set('last_autosave_time', datetime.datetime.now().timestamp())
